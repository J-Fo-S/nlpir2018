...
def getRegularizationValues():
    ...
    ### YOUR CODE HERE
    values = np.logspace(-1, 5, num=20, base=10)
    ### END YOUR CODE
...
def plotRegVsAccuracy(regValues, results, filename):
    ...
    plt.plot(regValues, [x["test"] for x in results])
    plt.plot(regValues, [x["dev"] for x in results])
    plt.xscale('log')
    plt.xlabel("regularization")
    plt.ylabel("accuracy")
    plt.legend(['train', 'test', 'dev'], loc='upper left')
...
def main(args):
    ...
    for reg in regValues:
        ...
        clf = MLPClassifier(solver='adam', alpha=1.0 / (reg + 1e-12), hidden_layer_sizes=(15,15,5), random_state=1)
        clf.fit(trainFeatures, trainLabels)

        # Test on train set
        pred = clf.predict(trainFeatures)
        #print(pred[:20], trainLabels[:20], trainFeatures[:20, :3])
    ...
    if args.pretrained:
        plotRegVsAccuracy(regValues, results, "q4_reg_v_acc_mlp.png")
